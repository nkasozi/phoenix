...
export default function DanekInstagramPostsShow(): JSX.Element {
	...
	return (
		...
		>
			<TextField
				value={translate("gathers.types.danek_instagram_posts.view.text")}
			/>
			<Space h="md" />
			<div className="flex gap-4 items-center mb-4">
				<GatherViewHeaderButton
					resource="danek_instagram_posts"
					id={id as string}
					projectid={projectid as string}
					record={record}
					isLoading={isLoading}
				...
				>
					...
					<Accordion.Item value="status" className="mb-4">
						<Accordion.Control>
							<Title order={5}>
								{translate(
									"gathers.types.danek_instagram_posts.view.accordion.status"
								)}
							</Title>
						</Accordion.Control>
						<Accordion.Panel>
					...
					<Accordion.Item value="general" mb="md">
						<Accordion.Control>
							<Title order={5}>
								{translate(
									"gathers.types.danek_instagram_posts.view.accordion.general"
								)}
							</Title>
						</Accordion.Control>
						<Accordion.Panel>
							<Container className="mx-0 flex flex-col my-4">
								...
								<Group>
									...
									<NumberField value={record?.scrape_comments_count_per_post} />
								</Group>
								<Group>
									<Title my="xs" order={5}>
										{translate(
					...
					<Accordion.Item value="source" mb="md">
						<Accordion.Control>
							<Title order={5}>
								{translate(
									"gathers.types.danek_instagram_posts.view.accordion.source"
								)}
							</Title>
						</Accordion.Control>
						<Accordion.Panel>
							<Container className="mx-0 flex flex-col my-4">
								<Group>
									<Title my="xs" order={5}>
										{translate(
											"gathers.types.danek_instagram_posts.view.accordion.source_title"
										)}
										:
									</Title>
								...
								{record?.account_username_list?.length > 0 && (
									<URLInputList
										list={record?.account_username_list}
